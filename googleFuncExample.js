...
const app = express();
const responso = (data) => {
  // this leaks to all sites that are visited by the client & it can block the client from accessing the proxy if
  // https is not avaliable.
...
// its deployed with npm:  "deploy": "gcloud functions deploy func-a --gen2 --runtime=nodejs16 --trigger-http --allow-unauthenticated --entry-point=funcA --source=. --region=europe-west1"
